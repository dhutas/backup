...
CHECK_NUM = 1000
check_num = 0
if os.path.exists('id_name_ver_desc_tags3_checkpoint.json'):
    id_name_ver_desc_tags = json.load(open('id_name_ver_desc_tags3_checkpoint.json','r',encoding='utf8'))
for id in range(800000,1200000): 
    ...
    try:
        ...
        if check_num%CHECK_NUM == 0:
            json.dump(id_name_ver_desc_tags, open('id_name_ver_desc_tags3_checkpoint.json','w',encoding='utf8'), ensure_ascii=False)
            print("[num = %d] Checkpoint saved to: id_name_ver_desc_tags3_checkpoint.json"%check_num)
    except Exception as e:
        #print("Error occured. Checkpoint saved to: id_name_ver_desc_tags_checkpoint.json -- ",e)
        print("[-] Error: ",e)
        continue
    
json.dump(id_name_ver_desc_tags, open('id_name_ver_desc_tags3.json','w',encoding='utf8'), ensure_ascii=False)
